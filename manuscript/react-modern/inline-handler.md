...
지금까지 살펴본 stories 리스트는 상태를 갖지 않은 변수였습니다. 검색 기능을 이용해 렌더링 된 리스트를 필터링할 수는 있지만, 필터를 제거하면 리스트 자체는 온전한 상태로 남아있습니다. 필터는 단지 서드파티를 이용해 일시적인 변화를 만들어낼 뿐이며 실제 리스트를 조작할 수는 없습니다.
